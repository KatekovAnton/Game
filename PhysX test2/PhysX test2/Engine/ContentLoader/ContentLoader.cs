...
/*
        public static UnAnimRenderObject WorldObjectFromDescription(
            LevelObjectDescription description, 
            PackList packs,
            StillDesign.PhysX.Scene scene,
            StillDesign.PhysX.ControllerManager controllerManager)
        {
            RenderObjectDescription rod = new RenderObjectDescription();
            rod = packs.GetObject(description.RODName, rod) as RenderObjectDescription;
            Material mat = loadMaterial(description.matname, packs);
            ...
            for (int i = 0; i < models.Length; i++)
            {
                ...
                for (int j = 0; j < modelsubsets.Length; j++)
                {
                    EngineMesh subsetmesh = EngineMesh.FromBuffers(packs.GetObjects(rod.LODs[i].subsets[j].MeshNames));
            ...
            StillDesign.PhysX.Actor ObjectActor = null;
            CollisionMesh ObjectRCCM = null;
            Logic.SceneGraph.OTBoundingShape bs = null;
            if (description.IsRCCMEnabled)
            {
                ObjectRCCM = new CollisionMesh();
                ObjectRCCM = packs.GetObject(description.RCCMName, ObjectRCCM) as CollisionMesh;
                bs = new Logic.SceneGraph.OTBoundingShape(ObjectRCCM);
            }
            else
                bs = new Logic.SceneGraph.OTBoundingShape(description.RCShapeSize);
            ...
            #endregion
            return new UnAnimRenderObject(
                models,
                rod.IsShadowCaster,
                rod.IsShadowReceiver,
